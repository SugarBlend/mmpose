...
def parse_args():
    ...
    parser.add_argument('--config', default="config-halpe.xml", help='Labeling Interface xml code file path')
    parser.add_argument('--input', default="C:/Users/Alexander/Downloads/project-3-at-2025-11-16-02-32-5ea948c8.json", help='Label Studio format JSON file path')
    parser.add_argument('--output', default="output.json", help='The output COCO format JSON file path')
...
class LSConverter:
    ...
    def convert_to_coco(self, input_json: str, output_json: str):
        ...
        for item_idx, item in enumerate(ann_list):
            ...
            image_name = Path(item["data"]["image"]).name
            image_id = len(images)
            ...
            for i in range(len(item["annotations"][0]['result'])):
                tmp = item["annotations"][0]['result'][i]
                if tmp["type"] == "rectanglelabels":
                    if not i:
                        cached_value = item["annotations"][0]["result"][i]
                    else:
                        item["annotations"][0]["result"][i], cached_value = cached_value, item["annotations"][0]["result"][i]

            item["annotations"][0]["result"].pop(0)
            item["annotations"][0]["result"].append(cached_value)

            keypoints_tensor = np.zeros((26, 3))
            ...
            for i, label in enumerate(item['annotations'][0]['result']):
                ...
                if 'rectanglelabels' == label['type'] or 'labels' == label[
                    ...
                    annotations[-1]['category_id'] = self.category_name_to_id["person"] + 1
                    ...
                    keypoints_tensor[category_id - 1] = [x, y, 2]
                    kp_num_change = 1
                    ...
                    if i == 0 or item['annotations'][0]['result'][
                        ...
                        annotations.append({
                            'id': annotation_id,
                            'image_id': image_id,
                            'keypoints': keypoints_tensor.flatten().tolist(),
                            'ignore': 0,
                            'iscrowd': 0,
                        })
                        kp_num = kp_num_change
                    else:
                        annotations[-1]['keypoints'] = keypoints_tensor.flatten().tolist()
                        kp_num += kp_num_change
